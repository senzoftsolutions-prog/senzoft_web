...
import { ImageHoverCard } from "../ui/ImageHoverCard";

export function FieldMedia({ slug }: { slug: string }) {
  const visual = getFieldVisual(slug);
  ...
  return (
    <div className={`grid items-stretch gap-5 ${layout}`}>
      {items.map((item) => {
        ...
        const visual = getFieldVisual(item.slug);
        ...
        const content = (
          <>
            ...
            <p className="mt-4 grow leading-7 text-brand-muted">{item.summary}</p>
        ...
        return (
          <div className="h-full" key={item.slug}>
            {visual || item.image ? (
              <ImageHoverCard
                to={href}
                image={visual?.src ?? item.image ?? "/media/digital-work.jpg"}
                alt={visual?.alt ?? ""}
              ...
              <Link to={href} className="card content-card group flex h-full min-h-64 flex-col p-7">
                {content}
              </Link>
            )}
          </div>
